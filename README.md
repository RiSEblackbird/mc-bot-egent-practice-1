...
2025 年 10 月時点の Mineflayer ではブロックへ完全一致しないと移動完了とみなされず、段差や水流の影響で「目的地付近に到達しているのに失敗扱い」になるケースが散見されました。本リポジトリでは `moveTo` コマンドを受け取った際に `GoalNear` を採用し、目的座標から ±3 ブロック以内なら完了として扱います。これにより、プレイヤー指定の XYZ 座標へ到達したにもかかわらず障壁報告が発生する不具合を低減できます。
