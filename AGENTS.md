...
- リポジトリ変更を伴う作業では、最終回答前に `Local completion`、`PR completion`、`Blocked`、`Review only` のどれかへ状態を分類する。
- `Local completion` では、作業ブランチ、変更差分、関連検証、未実行検証、残るリスク、計画ファイルの最終状態を確認する。
- ユーザーが commit / push / PR 作成を求めた場合、またはタスクの受け入れ条件に PR が含まれる場合は、commit 済み、push 済み、PR URL、最新 commit の CI 状態まで確認できるまで `PR completion` と言ってはいけない。
- Draft PR はレビュー待ちやリスク共有には使えるが、通常の完了扱いにしない。
- 最終回答には必要に応じて `Branch`、`PR URL`、`Commit SHA`、`Local verification`、`CI result`、`Remaining risks` を含める。該当しない項目は `N/A` として明示する。
...
- 途中成果の共有は計画ファイル更新とローカルコミットを基本とし、通常 PR は受け入れ条件達成後に作成する。早期共有が不可欠な場合のみ Draft PR を使う。
